...
function scan()
    ...
    obj.cpu_used = getCpu()
    obj.disk_used = getRootVolumes(false)
    obj.disk_free = getRootVolumes(true)
    ...
    obj.display_free_text = hs.styledtext.new(obj.disk_free ..'\n'.. 'FREE ' , {font={size=9}, color={hex='#FFFFFF'}, paragraphStyle={alignment="left", maximumLineHeight=18}})
    ...
    local canvas = hs.canvas.new{x = 0, y = 0, h = 24, w = 30+30+30+60+15}
    ...
    canvas:appendElements({
        ...
        -- withShadow = true,
        trackMouseEnterExit = true,
        frame = { x = 92, y = "0", h = "1", w = "1", }
        },{
        type = "text",
        text = obj.display_text,
        -- withShadow = true,
        trackMouseEnterExit = true,
        frame = { x = 115, y = "0", h = "1", w = "1", }
        })
...
function getRootVolumes(isFree)
    ...
    for key, vol in pairs(vols) do
        ...
        local freeReturn = free / 1024 / 1024 / 1024
        local usedSSD = (1-free/size) * 100
        if ( string.find(vol.NSURLVolumeNameKey,'Macintosh') ~= nil and isFree == false) then
            return formatPercent(usedSSD)
        elseif (string.find(vol.NSURLVolumeNameKey,'Macintosh') ~= nil and isFree == true) then
            return string.format("%.1f", freeReturn).."G"
        end
    end
    return ' 0%'
end
